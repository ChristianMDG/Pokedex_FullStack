...
export class PokemonController {
  ...
  async getPokemonList(req: Request, res: Response): Promise<void> {
    try {
      ...
      const result = await pokemonService.getPokemonList(safeLimit, offset);
      res.json(result);
  ...
  async searchPokemon(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!query || query.trim() === '') {
        res.status(400).json({ error: 'Veuillez fournir un terme de recherche' });
        return;
      }
  ...
  getCustomPokemonById(req: Request, res: Response): void {
    try {
      ...
      if (!pokemon) {
        res.status(404).json({ error: 'Pokémon personnalisé non trouvé' });
        return;
      }
  ...
  createCustomPokemon(req: Request, res: Response): void {
    try {
      ...
      if (!types || !Array.isArray(types) || types.length === 0) {
        res.status(400).json({ error: 'Les types sont requis' });
        return;
      }
      
      if (!sprite || typeof sprite !== 'string') {
        res.status(400).json({ error: 'L\'image est requise' });
        return;
      }
